...
contract EulerLiteFixed {
    ...
    // ── THE ONLY CHANGE FROM EulerLite ──
    //   BEFORE (vulnerable): donate collateral, NO health re-check  → you can self-sabotage
    //   AFTER  (fixed):      donate collateral, THEN require(healthy) → self-sabotage reverts
    function donateToReserves(uint256 amt) external {
        collateral[msg.sender] -= amt;
        reserves += amt;
        require(healthy(msg.sender), "unhealthy");   // ← THE FIX: you must stay solvent
    }
